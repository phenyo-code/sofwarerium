...
export async function GET() {
  ...
  const pages = [
    ...
    // Project Pages (updated to match your latest featuredProjects)
...
${pages.map(page => `<url><loc>${baseUrl}${page.url}</loc><lastmod>${page.lastmod}</lastmod><changefreq>${page.changefreq}</changefreq><priority>${page.priority}</priority></url>`).join('')}
</urlset>`.trim();
